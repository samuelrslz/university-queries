...
INSERT INTO courses
	(course_name, course_code, course_number, course_credits, dept_id)
VALUES 
	("Intro to Databases", "CIT 111", 111, 3, 1),
    ("Econometrics", "ECON 388", 388, 4, 2),
    ("Micro Economics", "ECON 150", 150, 3, 2),
    ("Classical Heritage", "HUM 376", 376, 2, 3);
...
FROM faculty
	JOIN sections
    ON faculty.faculty_id = sections.faculty_id
    JOIN semesters
    ON sections.semester_id = semesters.semester_id
    JOIN sections_has_students
    ON sections.section_id = sections_has_students.section_id
    JOIN students
    ON sections_has_students.student_id = students.student_id
WHERE students.fname = "Bryce" AND semesters.year = 2018 AND semesters.term = "Winter"
...
FROM students
    ...
    JOIN semesters
    ON sections.semester_id = semesters.semester_id
WHERE course_name = "Econometrics" AND semesters.year = 2019 AND semesters.term = "Fall"
ORDER BY lname;

-- Query 6

SELECT dept_code AS department_code, course_number AS course_num, course_name AS "name"
FROM departments
	JOIN courses
    ...
    JOIN semesters
    ON sections.semester_id = semesters.semester_id
WHERE students.fname = "Bryce" AND students.lname = "Carlson" AND semesters.term = "Winter"
ORDER BY course_name;

-- Query 7

SELECT term, year, COUNT(semesters.semester_id)
FROM semesters
	JOIN sections
    ON semesters.semester_id = sections.semester_id
    JOIN sections_has_students
    ON sections.section_id = sections_has_students.section_id
WHERE semesters.semester_id = 1;
...
SELECT faculty_fname AS fname, faculty_lname AS lname, SUM(section_capacity) AS TeachingCapacity
FROM faculty
	JOIN sections
    ON faculty.faculty_id = sections.faculty_id
    JOIN semesters
    ON sections.semester_id = semesters.semester_id
WHERE semesters.semester_id = 2
GROUP BY faculty.faculty_id
ORDER BY TeachingCapacity;

-- Query 10

SELECT lname, fname, SUM(course_credits) AS Credits
FROM students
	JOIN sections_has_students
    ON students.student_id = sections_has_students.student_id
    JOIN sections
